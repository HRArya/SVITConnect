...
def newUser(username, location, bio, mailid, website, firstname, lastname, photo, dateofbirth,password):
    conn = psycopg2.connect(
    ...
    try:
        cur.execute("insert into users values ('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}' , '{}', '{}','{}')".format(
            username, location, bio, mailid, website, firstname, lastname, photo, dateofbirth, joined_from,password))
...
def Auth(username):
    ...
    cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    query = "select mailid, password from users where username = '{}'".format(username)
    cur.execute(query)
    data = cur.fetchall()
    conn.commit()
    conn.close()
